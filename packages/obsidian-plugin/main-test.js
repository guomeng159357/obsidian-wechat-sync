...
`;
    ...
    var nodeHasCustomStyle = (node) => {
      ...
      for (let i = 0; i < properties.length; i++) {
        if (customStyleProperties.includes(properties[i]))
          return true;
      }
      return false;
    };
    var hasNonDefaultSpacingAttribute = (node, name) => {
      ...
      if (!normalisedValue)
        return false;
      if (normalisedValue === "0" || normalisedValue === "0px")
        return false;
      return true;
    };
    var nodeHasCustomAttributes = (node) => {
      ...
      if (node.nodeName === "TABLE") {
        if (hasNonDefaultSpacingAttribute(node, "cellpadding"))
          return true;
        if (hasNonDefaultSpacingAttribute(node, "cellspacing"))
          return true;
      }
      return false;
    };
    ...
    var tableHasCustomStyles = (tableNode) => {
      if (nodeHasCustomFormatting(tableNode))
        return true;
...
var WechatSyncSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian.Setting(containerEl).setName("\u6536\u4EF6\u7BB1\u8DEF\u5F84").setDesc("\u672C\u5730\u6536\u4EF6\u7BB1\u6587\u4EF6\u5939\u7684\u7EDD\u5BF9\u8DEF\u5F84").addText(
      (text) => text.setPlaceholder("/Users/xxx/sample-inbox").setValue(this.plugin.settings.inboxPath).onChange(async (value) => {
        this.plugin.settings.inboxPath = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("\u8F93\u51FA\u6587\u4EF6\u5939").setDesc("\u7B14\u8BB0\u5728 vault \u4E2D\u7684\u8F93\u51FA\u76EE\u5F55").addText(
      (text) => text.setPlaceholder("WeChat").setValue(this.plugin.settings.outputFolder).onChange(async (value) => {
        this.plugin.settings.outputFolder = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("\u9644\u4EF6\u6587\u4EF6\u5939").setDesc("\u9644\u4EF6\u5728 vault \u4E2D\u7684\u5B58\u50A8\u76EE\u5F55").addText(
      (text) => text.setPlaceholder("WeChat/attachments").setValue(this.plugin.settings.attachmentsFolder).onChange(async (value) => {
        this.plugin.settings.attachmentsFolder = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("\u8F6E\u8BE2\u95F4\u9694\uFF08\u6BEB\u79D2\uFF09").setDesc("\u6587\u4EF6\u76D1\u542C\u4E0D\u53EF\u7528\u65F6\u7684\u8F6E\u8BE2\u95F4\u9694").addSlider(
      (slider) => slider.setLimits(1e3, 6e4, 1e3).setValue(this.plugin.settings.pollingIntervalMs).setDynamicTooltip().onChange(async (value) => {
        this.plugin.settings.pollingIntervalMs = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("\u81EA\u52A8\u5F52\u6863").setDesc("\u5904\u7406\u5B8C\u6210\u540E\u81EA\u52A8\u5F52\u6863\u8BB0\u5F55").addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.autoArchive).onChange(async (value) => {
        this.plugin.settings.autoArchive = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("\u9ED8\u8BA4\u6807\u7B7E").setDesc("\u6DFB\u52A0\u5230\u6BCF\u7BC7\u7B14\u8BB0\u7684\u9ED8\u8BA4\u6807\u7B7E\uFF0C\u7528\u9017\u53F7\u5206\u9694").addText(
      (text) => text.setPlaceholder("wechat").setValue(this.plugin.settings.defaultTags.join(", ")).onChange(async (value) => {
        this.plugin.settings.defaultTags = value.split(",").map((t) => t.trim()).filter(Boolean);
        await this.plugin.saveSettings();
      })
    );
    ...
    new import_obsidian.Setting(containerEl).setName("\u6587\u5B57/\u94FE\u63A5\u6A21\u677F").setDesc("\u53D8\u91CF\uFF1A{{title}} {{date}} {{content}} {{sender}} {{tags}}").addTextArea(
      (text) => text.setValue(this.plugin.settings.templateText).onChange(async (value) => {
        this.plugin.settings.templateText = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("\u6587\u7AE0\u6A21\u677F").setDesc("\u989D\u5916\u53D8\u91CF\uFF1A{{url}} {{author}} {{publishDate}}").addTextArea(
      (text) => text.setValue(this.plugin.settings.templateArticle).onChange(async (value) => {
        this.plugin.settings.templateArticle = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("\u6587\u4EF6/\u56FE\u7247\u6A21\u677F").setDesc("\u989D\u5916\u53D8\u91CF\uFF1A{{fileType}} {{fileSize}} {{fileName}}").addTextArea(
...
var RECORD_FILE_PATTERN = /^\d{8}T\d{6}-[a-f0-9-]+\.json$/;
var TMP_SUFFIX = ".tmp";
function formatRecordFileName(timestamp, id) {
  const ts = timestamp.replace(/[-:]/g, "").replace(/\..+/, "").replace("T", "T");
  return `${ts}-${id}.json`;
}
function formatDateFolder(date = /* @__PURE__ */ new Date()) {
...
function validateInboxRecord(data) {
  const errors = [];
  ...
  if (!r.id || typeof r.id !== "string")
    errors.push("\u7F3A\u5C11 id");
  if (r.version !== "1.0")
    errors.push("version \u5FC5\u987B\u4E3A 1.0");
  if (!["text", "article", "file"].includes(r.type))
    errors.push("type \u65E0\u6548");
  ...
  if (!r.source || typeof r.source !== "object") {
    ...
    if (s.platform !== "wechat")
      errors.push("source.platform \u5FC5\u987B\u4E3A wechat");
    if (!s.receivedAt || typeof s.receivedAt !== "string")
      errors.push("\u7F3A\u5C11 source.receivedAt");
  }
    ...
    switch (r.type) {
      case "text":
        if (!c.text || typeof c.text !== "string")
          errors.push("text \u7C7B\u578B\u7F3A\u5C11 content.text");
        break;
      case "article":
        if (!c.url || typeof c.url !== "string")
          errors.push("article \u7C7B\u578B\u7F3A\u5C11 content.url");
        break;
      case "file":
        if (!c.fileName || typeof c.fileName !== "string")
          errors.push("file \u7C7B\u578B\u7F3A\u5C11 content.fileName");
        if (!c.sourcePath || typeof c.sourcePath !== "string")
          errors.push("file \u7C7B\u578B\u7F3A\u5C11 content.sourcePath");
        break;
    }
  }
...
var VaultWriter = class {
   ...
   * 创建 Markdown 笔记
   */
  async createNote(folder, fileName, content) {
    const folderPath = (0, import_obsidian2.normalizePath)(folder);
    await this.ensureFolder(folderPath);
    const safeName = sanitizeFileName(fileName);
    let finalPath = (0, import_obsidian2.normalizePath)(`${folderPath}/${safeName}.md`);
    if (await this.vault.adapter.exists(finalPath)) {
      let counter = 1;
      let altPath;
      do {
        altPath = (0, import_obsidian2.normalizePath)(`${folderPath}/${safeName} (${counter}).md`);
        counter++;
   ...
   * 创建二进制文件（图片、附件等）
   */
  async createBinaryFile(folder, fileName, data) {
    const folderPath = (0, import_obsidian2.normalizePath)(folder);
    await this.ensureFolder(folderPath);
    const safeName = sanitizeFileName(fileName);
    let finalPath = (0, import_obsidian2.normalizePath)(`${folderPath}/${safeName}`);
    if (await this.vault.adapter.exists(finalPath)) {
      ...
      let altPath;
      do {
        altPath = (0, import_obsidian2.normalizePath)(`${folderPath}/${base} (${counter})${ext}`);
        counter++;
      } while (await this.vault.adapter.exists(altPath));
      finalPath = altPath;
    }
   ...
   */
  async createDatedNote(outputFolder, fileName, content, date = /* @__PURE__ */ new Date()) {
    const datedFolder = (0, import_obsidian2.normalizePath)(`${outputFolder}/${formatDateFolder(date)}`);
    return this.createNote(datedFolder, fileName, content);
  }
};

// src/services/markdown-builder.ts
...
// src/services/html-converter.ts
...
var HtmlConverter = class {
  ...
  constructor() {
    ...
    this.turndown.use(import_turndown_plugin_gfm.gfm);
  ...
  convert(html) {
    if (!html || html.trim().length === 0)
      return "";
    const cleaned = html.replace(/<div[^>]*class="[^"]*reward[^"]*"[^>]*>[\s\S]*?<\/div>/gi, "").replace(/<div[^>]*class="[^"]*ad[^"]*"[^>]*>[\s\S]*?<\/div>/gi, "").replace(/<mp-common-profile[^>]*>[\s\S]*?<\/mp-common-profile>/gi, "").replace(/<div[^>]*id="js_pc_qr_code"[^>]*>[\s\S]*?<\/div>/gi, "");
    return this.turndown.turndown(cleaned);
  }
};

// src/services/attachment-copier.ts
var import_obsidian3 = require("obsidian");
var import_obsidian4 = require("obsidian");
var AttachmentCopier = class {
  constructor(vault) {
    this.vault = vault;
  }
  /**
   ...
   * 返回 vault 内的相对路径
   */
  async copyToVault(sourcePath, attachmentsFolder) {
    const fileName = sourcePath.split("/").pop() || "attachment";
    const safeName = sanitizeFileName(fileName);
    const data = await this.vault.adapter.readBinary(sourcePath);
    const folderPath = (0, import_obsidian4.normalizePath)(attachmentsFolder);
    await this.ensureFolder(folderPath);
    let finalPath = (0, import_obsidian4.normalizePath)(`${folderPath}/${safeName}`);
    if (await this.vault.adapter.exists(finalPath)) {
      const ext = safeName.includes(".") ? safeName.slice(safeName.lastIndexOf(".")) : "";
      const base = safeName.slice(0, safeName.lastIndexOf("."));
      let counter = 1;
      let altPath;
      do {
        altPath = (0, import_obsidian4.normalizePath)(`${folderPath}/${base} (${counter})${ext}`);
        counter++;
      } while (await this.vault.adapter.exists(altPath));
      finalPath = altPath;
    }
   ...
   * 下载远程图片到 vault
   */
  async downloadImage(url, attachmentsFolder) {
    ...
    const folderPath = (0, import_obsidian4.normalizePath)(attachmentsFolder);
    await this.ensureFolder(folderPath);
    const finalPath = (0, import_obsidian4.normalizePath)(`${folderPath}/${fileName}`);
    if (!await this.vault.adapter.exists(finalPath)) {
...
// src/services/article-fetcher.ts
var ArticleFetcher = class {
  /**
   * 抓取文章内容
   */
  async fetch(url) {
  ...
  extractPublishDate(html) {
    ...
    if (wxMatch) {
      const ts = parseInt(wxMatch[1], 10) * 1e3;
...
var RecordReader = class {
  constructor(inboxPath) {
    ...
    const _require = typeof require !== "undefined" ? require : globalThis.require;
    this.fs = _require("fs");
   ...
   */
  async moveToStatus(filePath, record, status, error) {
...
var InboxWatcher = class {
  constructor(inboxPath, recordReader, processorRegistry, pollingIntervalMs, errorHandler, onSyncStart, onSyncEnd) {
    this.inboxPath = inboxPath;
  ...
  async start() {
    try {
      ...
      this.watcher = chokidar.watch(pendingDir, {
        ignored: (path) => path.endsWith(TMP_SUFFIX),
        persistent: true,
        ignoreInitial: true,
        depth: 0
      });
    ...
    try {
      ...
      for (const file of files) {
        if (file.endsWith(TMP_SUFFIX))
          continue;
        if (!RECORD_FILE_PATTERN.test(file))
          continue;
  ...
  async processRecord(filePath) {
    try {
      ...
      const typeNames = { text: "\u6587\u5B57", article: "\u6587\u7AE0", file: "\u6587\u4EF6" };
...
// src/processors/processor-registry.ts
var ProcessorRegistry = class {
...
var TextProcessor = class {
  constructor(vaultWriter, markdownBuilder, outputFolder, defaultTags, template) {
...
// src/processors/article-processor.ts
var ArticleProcessor = class {
  constructor(vaultWriter, markdownBuilder, htmlConverter, attachmentCopier, articleFetcher, outputFolder, attachmentsFolder, defaultTags, template) {
    this.vaultWriter = vaultWriter;
    this.markdownBuilder = markdownBuilder;
  ...
  async createLinkFallback(record) {
    const content = record.content;
...
var IMAGE_TYPES = ["image/png", "image/jpeg", "image/gif", "image/webp", "image/svg+xml", "image/bmp"];
var FileProcessor = class {
  constructor(vaultWriter, markdownBuilder, attachmentCopier, outputFolder, attachmentsFolder, defaultTags, template) {
    this.vaultWriter = vaultWriter;
    this.markdownBuilder = markdownBuilder;
    this.attachmentCopier = attachmentCopier;
    this.outputFolder = outputFolder;
...
function isRetryable(error) {
  ...
  return message.includes("timeout") || message.includes("network") || message.includes("econnrefused") || message.includes("econnreset") || message.includes("etimedout") || message.includes("429") || message.includes("503");
}
var ErrorHandler = class {
  ...
  notify(message, duration = 5e3) {
    new import_obsidian5.Notice(`\u5FAE\u4FE1\u540C\u6B65: ${message}`, duration);
  }
  /**
   * 获取错误日志
   */
...
var WechatSyncPlugin = class extends import_obsidian6.Plugin {
  ...
  async onload() {
    ...
    this.statusBarEl.setText("\u5FAE\u4FE1\u540C\u6B65: \u5C31\u7EEA");
